...
class ParticleManager {
    ...
    /* init() {
        this.container = $('<div class="particles-container"></div>')
            .css({
                position: 'fixed',
                top: 0,
                left: 0,
    ...
    /* getParticle() {
        ...
        return this.createDynamicParticle();
    } */
    ...
    /* createDynamicParticle() {
        const $particle = $('<div class="particle particle-dynamic"></div>')
            .css({
                position: 'absolute',
                width: '4px',
                height: '4px',
    ...
    /* releaseParticle(particle) {
        ...
        if (particle.animation) {
            cancelAnimationFrame(particle.animation);
        ...
        if (particle.dynamic && this.activeParticles.filter(p => p.dynamic).length > 20) {
    ...
    clearCache() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const duration = (0.5 + Math.random() * 0.5) * life;

            const targetX = x + Math.cos(angle) * distance;
            const targetY = y + Math.sin(angle) * distance;

            this.animateParticle(particle, x, y, targetX, targetY, color, size, duration, speed);
        }
    } */
    ...
    /* animateParticle(particle, startX, startY, targetX, targetY, color, size, duration, speed) {
        ...
        const animate = () => {
            ...
            particle.animation = requestAnimationFrame(animate);
        };
    ...
    /* createLineClearEffect(row, cols, gridCells, color, intensity = 1) {
        const bursts = Math.floor(3 * intensity);

        for (let i = 0; i < bursts; i++) {
            const col = Math.floor(Math.random() * cols);
            const $cell = gridCells[row][col];
            const offset = $cell.offset();
            const centerX = offset.left + $cell.outerWidth() / 2;
            const centerY = offset.top + $cell.outerHeight() / 2;

            this.createEffect(centerX, centerY, color, {
    ...
    /* createLineClearEffect(row, cols, gridCells, color, intensity = 1) {
        ...
        for (let i = 0; i < bursts; i++) {
            ...
            // Récupérer l'élément DOM brut (si c'est un objet jQuery)
            const el = $cell[0] || $cell;
            const rect = el.getBoundingClientRect();

            // getBoundingClientRect donne des coords en viewport,
            // on les convertit en coords "document" si ton système de particules les attend comme ça.
        ...
        for (let r = 0; r < piece.shape.length; r++) {
            for (let c = 0; c < piece.shape[r].length; c++) {
                if (piece.shape[r][c]) {
                    ...
                    if (gridY >= 0 && gridY < gridCells.length && gridX >= 0 && gridX < gridCells[0].length) {
